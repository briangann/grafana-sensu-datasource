...
System.register(['./common'], function (_export, _context) {
  ...
  var getResponseForTarget, getClientsWithFilter;
    ...
    switch (aTarget.clientQueryMode) {
      ...
      case 'count':
        if (response.data.length > 0) {
          ...
          var _arrClientNames = [];
          if (aTarget.filters !== undefined && aTarget.filters.length > 0) {
            _arrClientNames = getClientsWithFilter(aTarget, response);
  ...
  function convertClientSummaryMetricsToJSON(aTarget, responses) {
    ...
    setters: [function (_common) {
      ...
      getClientsWithFilter = _common.getClientsWithFilter;
    }],
    execute: function () {
      _export('convertClientsToDataPoints', convertClientsToDataPoints);

      _export('convertClientsToJSON', convertClientsToJSON);

      _export('convertClientHistoryToDataPoints', convertClientHistoryToDataPoints);

      _export('convertClientSummaryMetricsToJSON', convertClientSummaryMetricsToJSON);

      _export('getClientsURIs', getClientsURIs);
    }
  };
});
